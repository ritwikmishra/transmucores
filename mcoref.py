...
def resolve_pronouns_hindi(text_list, model, hinlp):
    ...
    for doc in docs:
        ...
        for rn in replacements_needed[::-1]:
            ...
            replacee_string = re.sub(r'\s+',' ',' '.join(doc['cased_words'][replacee[0]:replacee[1]])).strip()
            replacer_string = re.sub(r'\s+',' ',' '.join(doc['cased_words'][replacer[0]:replacer[1]])).strip()
            replacer_string = add_karak(remove_karak(replacer_string), replacee_string)
            replacer_string = ' '.join([x for x in replacer_string.split() if x != 'एक'])
            if replacee[0] >= replacer[1]: # there should be no overlap
                doc['cased_words'] = doc['cased_words'][:replacee[0]] + [replacer_string] + ['']*(replacee[1]-replacee[0]-1) + doc['cased_words'][replacee[1]:]
...
def add_karak(text1, replacee_string):
    for k in ['ने','को','से','का','की','के','में','मे','पर','केलिए']:
        if re.search(k+r'$',replacee_string.strip()) and not (re.search(r'^(आ|अ)प',replacee_string.strip())) and replacee_string!='इसे':
            return text1+' '+k
    if replacee_string == 'उन्हें' or replacee_string == 'उन्हे' or replacee_string == 'इसे':
        return text1+' को'
    if re.search(r'^आप',replacee_string): # sentences written in second person are generally not resolved correctly
...
if __name__ == "__main__":
    ...
    try:
        hinlp = stanza.Pipeline('hi',dir=stanza_path+'/'+stanza_version+'/',download_method=2, use_gpu=use_gpu)
    except:
        stanza.download('hi', dir=stanza_path+'/'+stanza_version+'/') # model_dir or dir depending on the stanza version
        hinlp = stanza.Pipeline('hi',dir=stanza_path+'/'+stanza_version+'/',download_method=2, use_gpu=use_gpu)
    print('Stanza loaded')

    model = CorefModel("coref/config.toml", "xlmr")
    model.config.device = device
    model.load_weights(path="model/xlmr_multi_plus_hi2.pt", map_location=device,ignore={"bert_optimizer", "general_optimizer","bert_scheduler", "general_scheduler"})
    model.training = False

    raw_sentences = ['हमने कोहरे में एक आदमी को भागते देखा।  उसने काले रंग की शाल पहनी थी।  उसका कद लम्बा था। ']
    ...
    docs = resolve_coreferences(raw_sentences, model, hinlp)
    print(docs)
