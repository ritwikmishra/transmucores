...
class CorefModel:  # pylint: disable=too-many-instance-attributes
    ...
    def _build_optimizers(self):
        ...
        if self.config.bert_finetune:
            # my changes
            pass
            self.optimizers["bert_optimizer"] = torch.optim.Adam(self.bert.parameters(), lr=self.config.bert_learning_rate)
            self.schedulers["bert_scheduler"] = transformers.get_linear_schedule_with_warmup(self.optimizers["bert_optimizer"],n_docs, n_docs * self.config.train_epochs)

        # Must ensure the same ordering of parameters between launches
        # my changes
        modules = sorted((key, value) for key, value in self.trainable.items() if key != "bert")
        ...
        # my changes
        self.optimizers["general_optimizer"] = torch.optim.Adam(params, lr=self.config.learning_rate)
        self.schedulers["general_scheduler"] = transformers.get_linear_schedule_with_warmup(self.optimizers["general_optimizer"],0, n_docs * self.config.train_epochs)
    ...
    def _get_docs(self, path: str) -> List[Doc]:
        ...
        if path not in self._docs:
            ...
            if os.path.exists(cache_filename): # and 'train' in cache_filename: # new condition added
